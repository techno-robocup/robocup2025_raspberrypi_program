...
test_sequence = [
        (0000, 1),  # アーム2000, ワイヤON
        (2048, 0),  # アーム2500, ワイヤON
    ]
if __name__ == "__main__":
  try:
    while True:
      ...
      message = uart_io.receive_message()
      if message and message.getMessage() == "ON":
        for angle, wire in test_sequence:
            cmd = f"Rescue {angle}{wire}"
            logger.debug(f"[RPI] Sending: {cmd}")
            uart_io.send_message(
            Message(message_id, f"Rescue {int(angle)}{int(wire)}"))
            time.sleep(3)
